...
- Add model field encryption (`encryption` feature). AES-256-GCM via the `Encryptable` trait, with HKDF-SHA256 per-field key derivation, deterministic mode for queryable encrypted columns (`encrypt_query_value`), opt-in AAD binding (`aad_namespace = "..."` macro arg) to defeat ciphertext-relocation, zlib compression of large fields on by default with per-field opt-out (`(no_compress)` macro modifier), key rotation through `previous_keys`, automatic key-provider registration during `boot::create_context`, and context-aware helpers `encrypt_fields_ctx` / `decrypt_fields_ctx`. Generator support: `cargo loco generate model user ssn:string:encrypted email:string:encrypted:deterministic`. Envelope is Rails-inspired (`{p, h: {v, iv, at, i, d, c}}`) with versioning; from envelope `v2` the version and `d`/`c` flags are authenticated (folded into the AES-GCM associated data) so they can't be tampered with in storage. `loco_rs::Error` now implements `From<EncryptionError>` so handlers can use `?`. `impl_encryptable_fields!` supports both `String` and nullable `Option<String>` columns (the generator default). Saving a value encrypted under a `previous_keys` entry (or an older envelope version) transparently re-encrypts it with the current key — Rails' previous-encryption-schemes behavior — and saving an envelope no configured key can decrypt is an error. `aad_fields = [org_id]` binds ciphertexts to sibling row values (a tenant id) via `RowScope`, with `encrypt_query_value_scoped` / `encrypt_query_value_with` for deterministic queries on scoped models; `encrypt_field_with_aad` / `decrypt_field_with_aad` are re-exported. `Encryptable::provider_for` (macro arg `provider_for = path`) selects a per-row key provider from the scope, for per-tenant keys. The generator emits `text` for `:encrypted` string columns. Runnable demo at `examples/encryption/`. Docs: `/docs/the-app/encryption`.
