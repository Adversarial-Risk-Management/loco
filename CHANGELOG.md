...
- Add model field encryption (`encryption` feature). AES-256-GCM via the `Encryptable` trait, with HKDF-SHA256 per-field key derivation, deterministic mode for queryable encrypted columns (`encrypt_query_value`), opt-in AAD binding (`aad_namespace = "..."` macro arg) to defeat ciphertext-relocation, opt-in zlib compression for large fields (`(compress)` macro modifier), key rotation through `previous_keys`, automatic key-provider registration during `boot::create_context`, and context-aware helpers `encrypt_fields_ctx` / `decrypt_fields_ctx`. Generator support: `cargo loco generate model user ssn:string:encrypted email:string:encrypted:deterministic`. Envelope is Rails-inspired (`{p, h: {v, iv, at, i, d, c}}`) with versioning. `loco_rs::Error` now implements `From<EncryptionError>` so handlers can use `?`. Runnable demo at `examples/encryption/`. Docs: `/docs/the-app/encryption`.
