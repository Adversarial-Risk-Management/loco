...
pub enum ColumnValue {
    /// The column is `NotSet` and will not be written.
    NotSet,
    /// The column holds SQL `NULL` (`Option<String>` = `None`).
    Null,
    /// The column holds a string: plaintext to encrypt, or an envelope.
    Text(String),
}
...
pub trait Encryptable: ActiveModelTrait {
    ...
    ///
    /// Only consulted when at least one encrypted field holds a value, so
    /// partial updates that leave the scope columns `NotSet` are fine as long
    /// as they do not touch an encrypted column.
    ///
    /// # Errors
    ...
    /// Whether any scope column is `Set` on this `ActiveModel`.
    ///
    /// A changed scope changes every field's AAD, so the save must carry a
    /// value for every encrypted field; one left `NotSet` would keep a stored
    /// ciphertext that no longer decrypts.
    ...
    /// The current value of an encrypted column.
    ///
    /// Generated by the macro for every field in
    /// [`encrypted_fields`](Self::encrypted_fields); unknown names return
    /// [`ColumnValue::NotSet`].
    fn column_value(&self, field_name: &str) -> ColumnValue;
    ...
    where
        Self: Sized,
    {
        // Nothing to encrypt and no scope change to check: do not demand the
        // scope columns or a provider.
        if !self.scope_changed()
            && !Self::encrypted_fields()
                .iter()
                .any(|f| matches!(self.column_value(f), ColumnValue::Text(_)))
        {
            return Ok(self);
        }
    ...
    /// available.
    ///
    /// Every encrypted column that holds a value (`Set` or `Unchanged`) is
    /// processed; `NotSet` and `NULL` columns are left alone. A plaintext is
    /// encrypted. A value that is already an envelope is kept as-is when it
    /// is current, and **re-encrypted with the current scheme** when it was
    /// written under a previous key or a different deterministic mode —
    /// Rails' "previous encryption schemes" behavior. Rows therefore migrate
    /// to the newest key lazily as they are saved.
    ///
    /// # Errors
    /// Returns an error if encryption fails; if a value is an envelope that
    /// none of the configured keys can decrypt (persisting it would hide
    /// stale-key misconfiguration or a relocated ciphertext); or if a scope
    /// column is `Set` while an encrypted field is `NotSet` (the stored
    /// ciphertext would no longer decrypt under the new scope).
        ...
        Self: Sized,
    {
        ...
        for field_name in &fields {
            let value = match self.column_value(field_name) {
                ColumnValue::NotSet if scope_changed => {
                    return Err(EncryptionError::Scope(format!(
                        "a scope column is Set while encrypted field '{field_name}' is NotSet; \
                         set every encrypted field when changing a scope column, or the stored \
                         ciphertext will no longer decrypt"
                    )));
                }
                ColumnValue::NotSet | ColumnValue::Null => continue,
