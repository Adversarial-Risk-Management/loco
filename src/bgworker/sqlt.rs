...
impl QueueProvider for SqliteQueue {
    ...
    async fn enqueue_batch(
        ...
        enqueue_batch(&self.pool, &class, args_list, tags, priority).await
    }
...
mod tests {
    ...
    async fn can_enqueue_batch() {
        ...
        let ids = enqueue_batch(&pool, "BatchJob", args_list, None, None)
            .await
        ...
        let dequeued = dequeue(&pool, &[]).await.expect("dequeue ok");
        assert!(
            dequeued.is_some(),
            "a batched job must be dequeueable (run_at format regression)"
        );
    ...
    async fn can_enqueue_batch_across_chunks() {
        ...
        let args_list = (0..count)
            .map(|i| serde_json::json!({ "user_id": i }))
            .collect::<Vec<_>>();
