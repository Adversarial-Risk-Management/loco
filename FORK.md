...
| Multiple recipients in mailer (additive `MultiEmail`/`mail_multi`; `MultiMailerWorker` auto-registered in `boot.rs`) | `feat/mailer-multi` | — | [#1764](https://github.com/loco-rs/loco/pull/1764) | in review (mirrors upstream #1764; drop on the sync that merges it — keep only the `boot.rs` registration delta if upstream lands #1764 without it) |
