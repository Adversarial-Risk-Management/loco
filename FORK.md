...
| Scheduler runs registered tasks in process with the app's `AppContext` (jobs with `shell: true` still spawn a shell); native task args validated at scheduler setup | `push-tqkmmxzrnypn` | [#21](https://github.com/Adversarial-Risk-Management/loco/pull/21) | — | open (retargeted 1.1 → 1.2) |
| Coordinated graceful shutdown: SIGINT/SIGTERM through one app-wide token; HTTP, queue and scheduler ingress stop before active work drains; readiness fails during shutdown while health stays live; `Hooks::on_shutdown` runs once after the drain | `push-nsllxrnnyrmu` | [#22](https://github.com/Adversarial-Risk-Management/loco/pull/22) | — | open (stacked on #21; rebased onto 1.2) |
