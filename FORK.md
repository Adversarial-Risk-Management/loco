...
- **One branch per upstream version line carries the patches** — `0.16.x-arm`, `1.0.x-arm`, … Each
  starts at an upstream **release tag** (`1.2.x-arm` starts even with `v1.2.0`) and carries one
...
e.g. `v0.16.4-arm.1`, `v1.2.0-arm.1`. Tags are cut on the **version-line branch** tip. The
...
3. Same upstream version line (e.g. upstream tags `v1.0.1`): **merge the new release tag into
   ...
   line: start `M.x-arm` from the upstream release tag (`git checkout -b 1.2.x-arm v1.2.0`) and
   cherry-pick each still-needed patch commit from the previous line, one commit per patch.
4. Rebase open patch branches in the ledger onto the active line branch; run `cargo fmt --all`
   and `cargo clippy --workspace --all-features -- -D warnings -W clippy::pedantic -W
   clippy::nursery -W rust-2018-idioms` on each (this is the CI `style` gate; since 1.2 it runs
   on the toolchain pinned as `RUST_TOOLCHAIN` in `.github/workflows/loco-rs-ci.yml`, so use
   that, e.g. `cargo +1.98 clippy ...`). A patch that changes `loco-rs`'s public API must also
   regenerate the agent-skill API index (`cargo +nightly run -p xtask -- agent-skill`, which
   rewrites `skills/loco/api-index.md` and its copy under `loco-new/base_template/`) and fold
   it into the patch commit — the `docs` workflow runs `agent-skill --check` on every PR.
...
Current base: **upstream v1.2.0** (synced 2026-09-24). Patches carried on top, in commit order:

| Patch | Branch | Local PR | Upstream PR | Status |
|---|---|---|---|---|
| Fork docs + tooling | `fork-meta` | [#5](https://github.com/Adversarial-Risk-Management/loco/pull/5) | — | carried (re-applied on 1.2) |
| Default auth for GCP/Azure storage drivers (`new()` uses ambient creds; `with_credentials(...)` for explicit, credential last) | `feat/storage-default-auth` | [#14](https://github.com/Adversarial-Risk-Management/loco/pull/14) | candidate | carried (re-applied on 1.2 / opendal 0.58) |
| Testing deps: axum-test 20 (upstream pins 17; also drops the now-unrepresentable `RequestConfig.default_scheme`) | `deps/testing` | [#8](https://github.com/Adversarial-Risk-Management/loco/pull/8) | — | carried (residual — the rest of #8 landed upstream; upstream 1.2 still pins 17) |
| Queue provider downcast: `QueueProvider::as_any` + `Queue::downcast_provider::<T>()`, restoring app access to the queue backend (e.g. `PgQueue.pool`) that the 1.0 `Queue` enum removal took away | `feat/queue-provider-downcast` | — | candidate | carried (re-applied on 1.2; successor to `feature/expose-worker-job-ids` / `feat/work-queue-pagination`) |
| Testing: Postgres per-test DB teardown uses `DROP DATABASE ... WITH (FORCE)` (PG 13+), so app-held pools outside loco's own (e.g. a session store on a different sqlx major) can't fail cleanup with SQLSTATE 55006 | `fix/testing-drop-db-force` | — | candidate | carried (re-applied on 1.2) |
| loco-gen `:encrypted` field qualifier (string/text only, `^` only with deterministic, scaffold rejects it): migration/model output prints the `loco_encryption::impl_encryptable_fields!` block and config hint. The encryption runtime lives in the platform crate `loco-encryption`, not in the fork | `encryption-gen` | [#20](https://github.com/Adversarial-Risk-Management/loco/pull/20) | — | carried (merged on 1.1; re-applied on 1.2; supersedes [#19](https://github.com/Adversarial-Risk-Management/loco/pull/19), kept unmerged as the extraction source) |
| Scheduler runs registered tasks in process with the app's `AppContext` (jobs with `shell: true` still spawn a shell); native task args validated at scheduler setup | `push-tqkmmxzrnypn-1.2` | [#26](https://github.com/Adversarial-Risk-Management/loco/pull/26) | — | open (restacked on 1.2; was [#21](https://github.com/Adversarial-Risk-Management/loco/pull/21)) |
| Coordinated graceful shutdown: SIGINT/SIGTERM through one app-wide token; HTTP, queue and scheduler ingress stop before active work drains; readiness fails during shutdown while health stays live; `Hooks::on_shutdown` runs once after the drain | `push-nsllxrnnyrmu-1.2` | [#27](https://github.com/Adversarial-Risk-Management/loco/pull/27) | — | open (stacked on #26; was [#22](https://github.com/Adversarial-Risk-Management/loco/pull/22)) |

### Dropped on the v1.2.0 sync

Revivable from the `v1.1.0-arm.*` tags / `1.1.x-arm` if ever needed:

| Patch | Was | Why dropped |
|---|---|---|
| `perform_all_later` / `Queue::enqueue_batch` batch enqueueing | #3 (re-applied on 1.0/1.1) | Landed in 1.2 via upstream [#1821](https://github.com/loco-rs/loco/pull/1821). **The upstream API differs from ours:** `perform_all_later_with_priority(ctx, jobs)` takes `Vec<(A, Option<i32>)>` — a priority per job — instead of `(ctx, args_list, priority)`, and `Queue::enqueue_batch` / `QueueProvider::enqueue_batch` take `jobs: Vec<(args, Option<i32>)>` with no separate `priority` argument. Port old calls as `perform_all_later_with_priority(&ctx, args.into_iter().map(\|a\| (a, Some(p))).collect())`; `perform_all_later(ctx, args_list)` is unchanged. In `BackgroundQueue` mode with no queue provider it now returns `Error::QueueProviderMissing` instead of logging and fabricating ids (so does `perform_later_with_priority`) |
| 1.1 style-gate fixes (`fix(ci): make the 1.1 fork pass style checks`) | 1.1.x-arm-only commit | Upstream 1.2 pins CI to Rust 1.98 and passes `clippy --workspace` as-is. Its one still-needed hunk — opendal 0.58's `Operator::new` without `.finish()` in the GCP/Azure drivers — is folded into the storage patch |
