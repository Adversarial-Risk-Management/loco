...
| A hand-rolled `ApiContext` struct + `AddExtensionLayer`/`State` | `AppContext` (9 fields: `environment`, `db`, `queue_provider`, `config`, `mailer`, `storage`, `cache`, `shared_store`, `shutdown`), `#[derive(FromRef)]` | Same idea (one struct, threaded as Axum `State`) but pre-built with the pieces almost every service needs, and `FromRef`-derived so you can extract a single field (`State<DatabaseConnection>`) instead of always taking the whole context. See [AppContext and dependency injection](/docs/explanation/appcontext-and-di). |
