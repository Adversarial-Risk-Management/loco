    ...
    for col in column::columns_from_fields(fields)? {
        ...
        if let Some(mode) = col.encrypted {
            encrypted_fields.push(EncryptedField {
                name: col.name.clone(),
                deterministic: mode == column::EncryptionMode::Deterministic,
            });
        }
        match &col.kind {
...
mod tests {
    ...
    fn test_int_is_64_bit_big_integer() {
        ...
        let res = get_columns_and_references(&fields).expect("Failed to parse fields");
        assert_eq!(res, (vec![to_field("hits", "BigInteger")], vec![], vec![]));
    }
    ...
    fn test_big_int_is_64_bit_big_integer() {
        ...
        let res = get_columns_and_references(&fields).expect("Failed to parse fields");
        assert_eq!(res, (vec![to_field("views", "BigInteger")], vec![], vec![]));
    }
    ...
    fn test_reference_is_never_a_column_and_fk_is_64_bit() {
        ...
        assert_eq!(res, (vec![], vec![to_field("user", "")], vec![]));
        assert_eq!(
    ...
    fn test_decimal_required() {
        ...
        let res = get_columns_and_references(&fields).expect("Failed to parse fields");
        assert_eq!(res, (vec![to_field("price", "Decimal")], vec![], vec![]));
    }
}
